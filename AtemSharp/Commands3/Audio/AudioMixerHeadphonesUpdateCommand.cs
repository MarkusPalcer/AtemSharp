...
public class AudioMixerHeadphonesUpdateCommand : IDeserializedCommand
{
	...
	public static AudioMixerHeadphonesUpdateCommand Deserialize(Stream stream)
	{
		...
		{
			Gain = AtemUtil.UInt16BEToDecibel(reader.ReadUInt16BE()),
			ProgramOutGain = AtemUtil.UInt16BEToDecibel(reader.ReadUInt16BE()),
			TalkbackGain = AtemUtil.UInt16BEToDecibel(reader.ReadUInt16BE()),
			SidetoneGain = AtemUtil.UInt16BEToDecibel(reader.ReadUInt16BE())
		};
	}
