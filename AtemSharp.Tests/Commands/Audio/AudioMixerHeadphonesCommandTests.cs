	...
	AudioMixerHeadphonesCommandTests.CommandData>
{
	...
	protected override AudioMixerHeadphonesCommand CreateSut(TestCaseData testCase)
	{
		...
		var command = new AudioMixerHeadphonesCommand(CreateStateWithHeadphones());
	...
	/// <summary>
	/// Creates an AtemState with valid headphones configuration
	/// </summary>
	private static AtemState CreateStateWithHeadphones()
	{
	...
	{
		// Arrange
		var state = CreateStateWithHeadphones();
	...
	[Test]
	public void Constructor_WithNullHeadphones_ShouldInitializeToDefaultsAndSetAllFlags()
	{
		// Arrange
		var state = new AtemState
		{
	...
	public void Constructor_ShouldInitializeFromStateWithoutSettingFlags()
	{
		...
		Assert.That(command.ProgramOutGain, Is.EqualTo(-6.0));
		Assert.That(command.TalkbackGain, Is.EqualTo(-18.0));
		Assert.That(command.SidetoneGain, Is.EqualTo(-3.0));
		Assert.That(command.Flag, Is.EqualTo(0), "No flags should be set when initializing from state");
	}
