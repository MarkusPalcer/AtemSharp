...
public abstract class SerializedCommandTestBase<TCommand, TTestData> : CommandTestBase<TTestData>
	where TCommand : BasicWritableCommand
	where TTestData : SerializedCommandTestBase<TCommand, TTestData>.CommandDataBase, new()
{
	...
	public void TestSerialization(TestCaseData testCase)
	{
		...
		// Act
		var actualPayload = SerializeToByteArray(command, ProtocolVersion.V8_1_1);
		...
		Assert.Fail($"Commands3 serialization should match TypeScript reference for mask {testCase.Command.Mask:X2}. " +
	...
	/// <summary>
	/// Helper method to convert Stream result to byte array for comparison
	/// </summary>
	private static byte[] SerializeToByteArray(TCommand command, ProtocolVersion version)
	{
		using var stream = command.Serialize(version);
